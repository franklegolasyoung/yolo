...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            p = Path(p)  # to Path
            save_path = str('yoloimg.jpg')  # im.jpg
...
def draw_box_txt_fine(img_size, box, txt, font_path="./fonts/simfang.ttf"):
    box_height = int(
...
def display(img_path):
    ...
    im_show_table = draw_structure_result(
        image, table_result, font_path="/content/Code/fonts/simfang.ttf")
    ...
    cv2.imwrite("yoloocr.jpg", im_concat)
